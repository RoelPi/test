...
dt[,status := 'Fully Paid']
dt[loan_status != 'Fully Paid', status := 'Default']
dt[,year := year(issue_d)]
dt2 <- dcast.data.table(dt[,.(count = .N), by =.(grade, year, status)], 'grade + year ~ status')
dt2[,share_default := Default / (`Default` + `Fully Paid`) * 100]
kable(dt2[order(-share_default)])
